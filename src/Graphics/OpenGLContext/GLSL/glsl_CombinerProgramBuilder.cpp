...
public:
	ShaderTextureEngine(const opengl::GLInfo _glinfo) {
		m_part =
			...
			"highp vec4 clampWrapMirror(in highp vec4 vTexCoord, in highp vec4 vWrap,		\n"
			"	in highp vec4 vClamp, in lowp vec4 vWrapEn, in lowp vec4 vClampEn, in lowp vec4 vMirrorEn)		\n"
			"{																				\n"
			"	highp vec4 texCoord = vTexCoord;											\n"
			"	highp vec4 clampedCoord = clamp(texCoord, vec4(0.0), vClamp);				\n"
			//"	texCoord += vClampEn*(clampedCoord-texCoord);								\n"
			"	texCoord = mix(texCoord, clampedCoord, vClampEn); 							\n"
			"   lowp vec4 needMirror = step(vWrap, mod(texCoord, 2.0*vWrap));				\n"
			"	highp vec4 invertedCoord = -texCoord-vec4(1.0);								\n"
			//"	texCoord += vMirrorEn*needMirror*(invertedCoord-texCoord);	 				\n"
			"	texCoord = mix(texCoord, invertedCoord, vMirrorEn*needMirror); 				\n"
			"   highp vec4 wrappedCoord = mod(texCoord,vWrap);								\n"
			//"	texCoord += vWrapEn*(wrappedCoord-texCoord);								\n"
			"	texCoord = mix(texCoord, wrappedCoord, vWrapEn);							\n"
			"	return texCoord;															\n"
			"}																				\n"

			"highp vec4 wrap2D(in highp vec4 tc, in highp vec2 size)						\n"
			"{																				\n"
			"  highp vec2 divs = floor(tc.sp / size.ss);									\n"
			"  highp vec2 divt = floor((tc.tq + divs) / size.tt);							\n"
			"  return vec4(tc.sp - divs * size.ss, tc.tq + divs - divt*size.tt).sptq;		\n"
			"}																				\n"
			...
			"  highp vec4 tc = clampWrapMirror(intPart.stst + vec4(0.0,0.0,1.0,1.0), uTexWrap0.stst, uTexClamp0.stst, uTexWrapEn0.stst, uTexClampEn0.stst, uTexMirrorEn0.stst); \n"
			//"  tc = wrap2D(tc, uTexSize0);											\n"
			//"  highp vec4 tc = vec4(1.0);"
			"  tcData[0] = tc.st;													\n"
			"  tcData[3] = tc.pq;													\n"
			"  tcData[1] = tc.sq;													\n"
			"  tcData[2] = tc.pt;													\n"
			...
			"{																		\n"
			"  mediump vec2 intPart = floor(texCoord);								\n"
			"  highp vec4 tc = clampWrapMirror(intPart.stst + vec4(0.0,0.0,1.0,1.0), uTexWrap1.stst, uTexClamp1.stst, uTexWrapEn1.stst, uTexClampEn1.stst, uTexMirrorEn1.stst); \n"
			//"  tc = wrap2D(tc, uTexSize1);											\n"
			//"  highp vec4 tc = vec4(1.0);"
			"  tcData[0] = tc.st;													\n"
			"  tcData[3] = tc.pq;													\n"
			"  tcData[1] = tc.sq;													\n"
